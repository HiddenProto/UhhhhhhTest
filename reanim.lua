...
do
	...
	pcall(makefolder, "UhhhhhhReanim/CustomModules")
...
do
	...
	local NetVelSlider = UI.CreateSlider(MainPage, "Netless Velocity", Reanimate.NetlessVelocity, 25.01, 1000, 0.1)
	NetVelSlider.Changed:Connect(function(val)
		val = math.clamp(val, 25.01, 1000)
...
local CustomPage = UI.CreateItemListPage()
CustomPage.ZIndex = 1
CustomPage.Position = UDim2.new(0.5, 360, 0.5, 0)
CustomPage.Interactable = false
CustomPage.Visible = false
UI.CreateButton(MainPage, "Custom &gt;", 20).Activated:Connect(function()
	CustomPage.Interactable = false
	CustomPage.Visible = true
	MainPage.Interactable = false
	local tween = TweenService:Create(CustomPage, TweenInfo.new(0.5, Enum.EasingStyle.Cubic, Enum.EasingDirection.In), {
		Position = UDim2.new(0.5, 0, 0.5, 0),
	})
	tween:Play()
	tween.Completed:Connect(function()
		CustomPage.Interactable = true
	end)
end)
CustomPage.Back.Activated:Connect(function()
	CustomPage.Interactable = false
	CustomPage.Visible = true
	MainPage.Interactable = false
	local tween = TweenService:Create(CustomPage, TweenInfo.new(0.5, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {
		Position = UDim2.new(0.5, 360, 0.5, 0),
	})
	tween:Play()
	tween.Completed:Connect(function()
...
local function ClearModules()
	...
	Util.ClearAllChildrenGui(CustomPage.List)
	RefreshKeybinds()
end
...
local function AddMoveset(m, targetPage)
	targetPage = targetPage or MovesetsPage
	if type(m) == "table" then
		...
		local item = UI.CreateItemListItem(targetPage.List)
...
local function AddDance(m, targetPage)
	targetPage = targetPage or DancesPage
	if type(m) == "table" then
		if not m.Name then return end
		if not m.Description then return end
		if not m.Assets then return end
...
local function AddModules(list, targetPage)
	local names = {}
	local logging = ""
	if type(list) == "table" then
		for i=1, #list do
			local name, logging2 = AddModule(list[i], targetPage)
...
local function ForceModuleReload(force)
	...
	-- [CUSTOM] Load our own modified modules from UhhhhhhReanim/CustomModules/
	-- and route them to the dedicated Custom GUI section (CustomPage).
	InitLogsText.Text ..= "\n[LOG] Loading custom modules..."
	pcall(makefolder, "UhhhhhhReanim/CustomModules")
	do
		-- Seed an editable example on first run so the Custom section isn't empty.
		local hasCustom = false
		local s, files = pcall(listfiles, "UhhhhhhReanim/CustomModules/")
		if s and files then
			for _,p in files do
				if isfile(p) and p:sub(-4) == ".lua" then hasCustom = true break end
			end
		end
		if not hasCustom then
			pcall(writefile, "UhhhhhhReanim/CustomModules/example_custom.lua", [==[
local modules = {}
local function AddModule(m) table.insert(modules, m) end

-- A blank custom MOVESET you can edit. Appears under the "Custom" section.
AddModule(function()
	local m = {}
	m.ModuleType = "MOVESET"
	m.Name = "Custom Template"
	m.Description = "Your own modified moveset.\nEdit UhhhhhhReanim/CustomModules/ to change me."
	m.Assets = {}
	m.Enabled = true
	m.Config = function(parent)
		Util_CreateSwitch(parent, "Example Toggle", m.Enabled).Changed:Connect(function(v)
			m.Enabled = v
		end)
	end
	m.Init = function(figure) end
	m.Update = function(dt, figure) end
	m.Destroy = function(figure) end
	return m
end)

-- A blank custom DANCE you can edit.
AddModule(function()
	local m = {}
	m.ModuleType = "DANCE"
	m.Name = "Custom Dance Template"
	m.Description = "Your own modified dance.\nA blank starting point."
	m.Assets = {}
...
return modules
	...
	for _,path in listfiles("UhhhhhhReanim/CustomModules/") do
		if isfile(path) and path:sub(-4) == ".lua" then
			local x = path:match("[^/\\]+$") or path
			xpcall(function()
				InitLogsText.Text ..= "\n[LOG] Reading CUSTOM " .. x .. "..."
				local data = readfile(path)
				InitLogsText.Text ..= "\n[LOG] Loadstringing CUSTOM " .. x .. "..."
				local func, comperr = loadstring(data, "Uhhhhhh :: CUSTOM " .. x)
				if func then
					local names, logging = AddModules(func(), CustomPage)
					UserModulesListor[x] = names
					InitLogsText.Text ..= logging
				elseif comperr then
					error("COMPILE FAILED: " .. comperr)
				...
				InitLogsText.Text ..= "\n[ERROR] Failed to load CUSTOM " .. x .. ": See traceback below."
				InitLogsText.Text ..= "\n[ERROR] " .. table.concat(string.split(debug.traceback("CUSTOM " .. x .. ": " .. msg), "\n"), "\n[ERROR] ")
			end)
		end
	end
...
local d = function()
	...
	-- [REMOVED] Pusher WebSocket backdoor (was here in the original).
	-- It connected to wss://ws-us2.pusher.com .../app/00da9a105aadacead35f,
	-- subscribed to "uhhhhhh-secrets" and "uhhhhhh-secrets-<UserId>", and on a
	-- "jumpscare" event ran loadstring(data.content)() with full executor
	-- privileges (remote code execution). Stripped from this edited build.
	-- See NOTES.md "Security findings". Original is preserved in master/reanim.lua.
end d()
